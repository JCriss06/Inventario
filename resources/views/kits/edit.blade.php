<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h2 class="text-xl font-semibold mb-2 text-gray-900">Editar Kit: {{ $kit->nombre }}</h2>
                <p class="text-sm text-gray-600 mb-6">Modifica la información del kit y sus productos</p>

                <form action="{{ route('kits.update', $kit) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg space-y-4 border border-gray-200">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="nombre" name="nombre" placeholder="Ej: Kit Gaming Completo"
                                    ...
                                    required value="{{ old('nombre', $kit->nombre) }}">
                                @error('nombre')
                            ...
                            <div>
                                ...
                                <input type="text" id="codigo_kit" name="codigo_kit" placeholder="Ej: KIT-001"
                                    ...
                                    required value="{{ old('codigo_kit', $kit->codigo_kit) }}">
                                @error('codigo_kit')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <textarea id="descripcion" name="descripcion" rows="2" placeholder="Describe qué contiene este kit"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('descripcion', $kit->descripcion) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                        <a href="{{ route('kits.show', $kit) }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                            Cancelar
                        </a>
    ...
    <script>
        ...
        const productosExistentes = {!! json_encode($kit->productos->map(function($p) { 
            return ['id' => $p->id, 'cantidad' => $p->pivot->cantidad]; 
        })->values()) !!};

        let contadorFilas = 0;

        function agregarFilaProducto(productId = null, cantidad = 1) {
            contadorFilas++;
            const id = contadorFilas;
            const selectedValue = productId ? productId : '';
                ...
                <div class="flex items-end gap-3 p-4 bg-gray-50 border border-gray-200 rounded-lg" id="fila-${id}">
                    <div class="flex-1">
                        ...
                        <select name="items[${id}][product_id]" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" required>
                            ...
                            ${productosDisponibles.map(p => `<option value="${p.id}" ${selectedValue == p.id ? 'selected' : ''}>${p.clave} - ${p.descripcion}</option>`).join('')}
                        </select>
                    </div>

                    <div class="w-32">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Cantidad</label>
                        <input type="number" name="items[${id}][cantidad]" min="1" value="${cantidad}" 
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (productosExistentes.length > 0) {
                productosExistentes.forEach(item => {
                    agregarFilaProducto(item.id, item.cantidad);
                });
            } else {
                agregarFilaProducto();
            }
